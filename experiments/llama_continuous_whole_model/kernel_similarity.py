...
def _categorical_distribution(
    ...
    key: Callable[[Event], Hashable],
    weight: Callable[[Event], float],
) -> dict[Hashable, float]:
...
def _numeric_histogram(
    values: Sequence[float], upper_log: float, bins: int = 64
) -> dict[int, float]:
    if not values:
        return {}
...
def continuous_kernel_similarity(
    left: Sequence[Event], right: Sequence[Event]
) -> dict[str, float]:
    """Compare indefinitely repeated kernel streams without iteration alignment."""

    family_duration = _js_similarity(
        _categorical_distribution(
            left,
            lambda event: event["family"],
            lambda event: event["duration_us"],
        ),
        _categorical_distribution(
            right,
            lambda event: event["family"],
